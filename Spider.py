...
class SpiderMain(object):
    ...
    def __init__(self):
        ...
        self.connection = pymysql.Connect(host='127.0.0.1', user='root', passwd='zh20110612', charset="utf8")
    ...
    def WTDB_siteinfo(self, data):
        self.con_cursor.execute("INSERT INTO site_table_%s" % self.now + " \
            ...
            data['CO'], data['O3_24h'], data['PrimaryPollutant'],
            data['Quality']))

    # 写入城市信息到数据库
    def WTDB_cityinfo(self, data):
        self.con_cursor.execute("INSERT INTO city_table_%s" % self.now + " \
                                                    (`cityID`, `Time`,`AQI`,`PM2_5_24h`,`PM10_24h`,`SO2_24h`,`NO2_24h`,`CO_24h`,`O3_8h_24h`,`PrimaryPollutant`,`Quality`) \
                                                    VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
            data['CityCode'], data['Time'], data['AQI'],
            data['PM2_5_24h'], data['PM10_24h'], data['SO2_24h'], data['NO2_24h'],
            data['CO_24h'], data['O3_8h_24h'], data['PrimaryPollutant'],
            data['Quality']))
    ...
    def dealWithSiteData(self, dict):
        ...
        list = ['AQI', 'PM2_5', 'PM10', 'SO2', 'NO2', 'O3_24h']
        for index in list:
            try:
                data[index] = int(dict[index])
            except:
    ...
    def getAllCityInfo(self):
        ...
        for key in self.cityDict.keys():
            ...
            # 排除没有具体数据的情况
            try:
                data['CO_24h'] = float(latest_data['CO_24h'])
            except:
                data['CO'] = 0.0

            list = ['AQI', 'NO2_24h', 'O3_8h_24h', 'PM10_24h', 'PM2_5_24h', 'SO2_24h']
            for index in list:
                try:
                    data[index] = int(latest_data[index])
                except:
                    data[index] = 0
